...
public class Quaternion {
    ...
    public Quaternion(Vector3 v) { // v->euler angle
        e.x = v.x;
        e.y = v.y;
        e.z = v.z;
    ...
    public static Quaternion fromAxisWithAngle(Vector3 axis, float angle) {
        float halfAngle = angle / 2;
        float s = (float) Math.sin(halfAngle);
        return new Quaternion((float) Math.cos(halfAngle), axis.x * s, axis.y * s, axis.z * s);
    }

    public Quaternion rotateByAxis(Vector3 axis, float angle) {

        Vector3 x_axis = new Vector3(1, 0, 0);
        Vector3 initFacing = this.rotateVector(x_axis);
        Vector3 finalFacing = Quaternion.fromAxisWithAngle(axis, angle).rotateVector(initFacing);

        Vector3 finalAxis = x_axis.cross(finalFacing);
        float finalAngle = x_axis.dot(finalFacing) / (x_axis.norm() * finalFacing.norm());
        return Quaternion.fromAxisWithAngle(finalAxis, finalAngle);
    }
